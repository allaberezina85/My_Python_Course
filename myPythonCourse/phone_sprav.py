...
# функция копирования из файла file_name в файл file_name_c 
# строки под номером введенным пользователем в терминале
def copy_row_to_file(file_name, file_name_c):
    search = int(input('Введите номер строки для копирования: '))
    res = read_file(file_name)
    if search <= len(res):
        res_copy = res[search-1:search]
        standart_write(file_name_c, res_copy)
    else:
        print('Введен неверный номер строки')


file_name = 'phone.csv' # основной файл справочника
file_name_c = 'phone_c.csv' # дополнительный файл для функции копирования
...
def main():
    while True:
        ...
        # команда копирования в файл 
        elif command == 'c':
            if not exists(file_name):
                print('Файл отсутсвует, пожалуйста создайте файл')
                continue
            if not exists(file_name_c):
                create_file(file_name_c)
            copy_row_to_file(file_name, file_name_c)
